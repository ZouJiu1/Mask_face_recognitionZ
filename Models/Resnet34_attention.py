...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, showlayer= False, num_classes=1000, zero_init_residual=False,
        ...
        if self.showlayer:
            ...
            for level in [x]:
                ...
                if torch.cuda.is_available():
                    ...
                    # level = np.array(255 * level.detach().cpu().numpy(), dtype=np.uint8).copy()
                else:
                    level = np.array(255 * unnormalize(level).detach().numpy()).copy()
                    # level = np.array(255 * level.detach().numpy(), dtype=np.uint8).copy()
                level = np.transpose(level, (1, 2, 0))
                plt.imsave(os.path.join(pwd, 'Layer_show',  'V8.jpg'), level[:, :, 0])
